...
//
    public function getHtmlItem(Song $song, String $size) : string {
        ...
        $htmlitem = '<div class="col 12 m2 margin-bottom"><span class="pointer" onclick="activateTab(\'view/DiscoverView.php\')"><div class="card-2 grayscale-40"><img src="audiocity/'.$song->getPiclink().'" class="discover-img" alt="{album} Cover"><div class="container"><h4>'.$titel.'</h4><p class="opacity">{album}</p><p><span>Länge: '.$song->getLengthFormatted().'</span><span class="discover-date">Erschien: '.$song->getDate().'</span></p></div></div></span></div>';
